...
Use a shared-spine and complementary-coverage design. Suitable pairings include dividing examples along one continuum, applying opposite perspectives or reagents, comparing two categories, giving one group breadth and the other depth on the same principle, or using an identical anchor question. Where questions differ, they must remain on the same underlying concept. Across the complete activity, at least 40% of the numbered questions must use genuinely different Set A and Set B tasks. These variants must use different source-supported examples, cases or supplied data, not superficial rewording, while remaining matched in concept, command word, response form and workload.
...
Generate calculation questions only when the uploaded syllabus is primarily a mathematics syllabus. For science and every other subject, do not ask students to calculate or determine a numerical value, even when worked calculations appear in the uploaded materials. Prefer source-supported conceptual, descriptive, explanatory, comparative, classification, interpretation, equation, graph or diagram tasks instead. Every permitted calculation must show all required numerical inputs in the question or explicitly refer to a student-visible table, graph or diagram containing them; never place essential givens only in the compiled answer.
...
Make the answer format exactly match the displayed command. For a pure Sketch, Plot or Draw question, the completed editable graph or diagram is the answer; do not place an unasked Describe or Explain answer beside it. If students must also describe or explain the visual, ask that explicitly as a separate structured question in both Set A and Set B. Use a table response only when the visible question explicitly asks students to complete, fill in, populate or construct that table, or explicitly asks them to identify, deduce or interpret something from student-visible data in the table. When a task refers to supplied observations, results or data, prefill those values in the student table and leave only the required response cells blank. Never claim that information is provided when the student slide does not display it. The compiled debrief must reproduce the same table with every answer cell completed. A Describe or Explain question must use a text answer unless it explicitly asks for a table as well.
